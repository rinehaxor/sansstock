    ...
    for (const ipoData of ipoDataArray) {
      try {
        ...
        if (typeof underwriters === "string") {
          underwriterNames = underwriters.split(/[;,]/).map((name) => name.trim()).filter((name) => name.length > 0);
        ...
        for (const underwriterName of underwriterNames) {
          const { data: existingUnderwriter } = await authenticatedClient.from("underwriters").select("id").eq("name", underwriterName).single();
          if (existingUnderwriter) {
            underwriterIds.push(existingUnderwriter.id);
          } else {
            const { data: newUnderwriter, error: underwriterError } = await authenticatedClient.from("underwriters").insert({ name: underwriterName }).select().single();
            if (newUnderwriter && !underwriterError) {
              underwriterIds.push(newUnderwriter.id);
            }
          }
        }
        const { data: existingListing } = await authenticatedClient.from("ipo_listings").select("id").eq("ticker_symbol", ticker_symbol).single();
        let ipoListingId;
        if (existingListing) {
          const { data: updatedListing, error: updateError } = await authenticatedClient.from("ipo_listings").update({
            company_name,
            ipo_date,
          ...
          }).eq("ticker_symbol", ticker_symbol).select().single();
          if (updateError) {
            results.failed++;
          ...
          const { data: newListing, error: insertError } = await authenticatedClient.from("ipo_listings").insert({
            ticker_symbol,
            company_name,
            ipo_date,
            general_sector: general_sector || null,
        ...
        if (performance_metrics.length > 0) {
          const metrics = performance_metrics.filter((metric) => metric.metric_value !== null && metric.metric_value !== void 0).map((metric) => ({
            ipo_listing_id: ipoListingId,
