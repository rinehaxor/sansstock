  ...
  try {
    ...
    let query = supabase.from("ipo_listings").select(
      `
         ...
         `,
      { count: "exact" }
    ).order("ipo_date", { ascending: false }).range((page - 1) * limit, page * limit - 1);
    if (tickerSymbol) {
...
const POST = async (context) => {
  ...
  try {
    ...
    const { data: ipoListing, error: ipoError } = await authenticatedClient.from("ipo_listings").insert({
      ticker_symbol,
      company_name,
      ipo_date,
