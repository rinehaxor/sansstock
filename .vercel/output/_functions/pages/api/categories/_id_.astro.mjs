...
const GET = async (context) => {
  try {
    ...
    const { data: category, error } = await supabase.from("categories").select("id, name, slug, description, created_at, updated_at").eq("id", id).single();
    if (error) {
...
const PUT = async (context) => {
  ...
  try {
    ...
    if (slug) {
      const { data: existingCategory } = await authenticatedClient.from("categories").select("id").eq("slug", slug).neq("id", id).single();
      if (existingCategory) {
    ...
    const { data: category, error: categoryError } = await authenticatedClient.from("categories").update(updateData).eq("id", id).select().single();
    if (categoryError) {
...
const DELETE = async (context) => {
  ...
  try {
    ...
    const { data: articles, error: articlesError } = await authenticatedClient.from("articles").select("id").eq("category_id", id).limit(1);
    if (articlesError) {
