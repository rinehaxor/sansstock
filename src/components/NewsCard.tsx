...
export default function NewsCard({ title, slug, summary, thumbnail_url, thumbnail_alt, published_at, created_at, categories }: NewsCardProps) {
   ...
   return (
      <article className="group bg-white rounded-xl border border-gray-200 p-6 hover:shadow-md transition-shadow">
         <a href={articleUrl} className="flex gap-6">
            <div className="flex-shrink-0">
               {thumbnail_url ? (
                  <img src={thumbnail_url} alt={thumbnail_alt || title || 'Article thumbnail'} className="w-48 h-28 rounded-lg object-cover" loading="lazy" />
               ) : (
            ...
            <div className="flex-1 min-w-0">
               <h3 className="text-xl font-bold text-gray-900 group-hover:text-blue-600 mb-2 leading-tight">{title}</h3>
