...
export default function CategoryNewsList({ 
   ...
   const fetchArticles = async (page: number) => {
      setLoading(true);
      try {
         const response = await fetch(`/api/articles?page=${page}&limit=${limit}&status=published&category_id=${categoryId}`);
         if (response.ok) {
            ...
            // Scroll to category news list section
            const newsSection = document.getElementById('category-news-list');
            if (newsSection) {
   ...
   useEffect(() => {
      ...
      const newsSection = document.getElementById('category-news-list');
      const clientSection = document.getElementById('category-news-list-client');
   ...
   return (
      <>
         ...
         {isClient && currentPage !== initialPage && articles.length > 0 && !loading && (
            <div id="category-news-list-client" className="space-y-6">
